    ...
    def _inOrderRecursively(self, root, elements):
        if root:
            if not root.left and not root.right:
                return True
            left = self._inOrderRecursively(root.left, elements) if root.left else False
            right = self._inOrderRecursively(root.right, elements) if root.right else False
            if self._getNodeBalance(root) == 0 and left and right:
                elements[0] = self._getNodeHeight(root) if self._getNodeHeight(root) > elements[0] else elements[0]
                return True
            return False
...
# Problem 6: Mayor Subarbol Completo
